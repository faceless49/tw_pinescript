...
`;
  ...
  workbook.SheetNames.filter((sheetName) => sheetName !== 'Легенда').forEach((sheetName) => {
    ...
    rows.forEach((row) => {
      ...
      if (!tRaw) {
        console.log(`Пропущена строка без тикера (лист: ${sheetName})`);
        return;
      }
      const t = mapExcelTickerToPine(tRaw);
      ...
      console.log(
        `Excel: ${tRaw} -> Pine: ${t} | Цель1: ${g1}, Цель2: ${g2}, Стоп: ${s}, Отмена: ${c}, Вход: ${e} (лист: ${sheetName})`
      );
...
// Показываем только последнее значение (чтобы не затягивать историю)
...
// Смещение вправо на N баров по текущему таймфрейму
futureMs = int(timeframe.in_seconds()) * 1000 * labelsOffsetBars
xRight   = timenow + futureMs

// Рисуем/обновляем лейблы на каждом баре
// (чтобы при любом зуме/скролле и изменении окна они пересчитывались)
if true
    // Чистим прошлые
    if not na(lbl_goal1)
...
// Stop / Cancel: условия срабатывания (при необходимости адаптируй под лонг/шорт)
